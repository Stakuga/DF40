    ...
    def __getitem__(self, index, no_norm=False):
        ...
        try:
            if not os.path.exists(image_paths):
                image_paths = image_paths.replace('/Youtu_Pangu_Security_Public/', '/Youtu_Pangu_Security/public/')
        except:
            pass
        ...
        for image_path in image_paths:
            if not os.path.exists(image_path):
                image_path = image_path.replace('/Youtu_Pangu_Security_Public/', '/Youtu_Pangu_Security/public/')
            ...
            #         landmarks = None
            #         landmarks_trans = None


            # os.makedirs('./roop/', exist_ok=True)
            # cv2.imwrite(f"./roop/{image_path.split('/')[-1]}", image_trans)
...
if __name__ == "__main__":
    ...
    with open('/Youtu_Pangu_Security/public/youtu-pangu-public/zhiyuanyan/DeepfakeBenchv2/training/config/test_config.yaml', 'r') as f:
